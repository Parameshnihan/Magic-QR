...
async function seed() {
  ...
  const clients = await db.insert(clientsTable).values([
    {
      clientId: `CLT-${nanoid(8).toUpperCase()}`,
      name: "Sophia Laurent",
  ...
  const campaigns = await db.insert(qrCampaignsTable).values([
    {
      ...
      destinationUrl: `https://reviewflowpro.app/review/`,
      status: "active",
      totalScans: 1134,
      totalReviews: 342,
      conversionRate: 30.2,
  ...
  await db.insert(feedbackTable).values([
    ...
    },
    {
      ...
      clientId: clients[2].id,
      rating: 1,
      feedbackText: "My therapist showed up 25 minutes late for my 60-minute appointment and the session was still cut short. Unacceptable.",
  ...
  await db.insert(invoicesTable).values([
    { invoiceNumber: `INV-2026-${nanoid(6).toUpperCase()}`, clientId: clients[0].id, subscriptionId: subs[0].id, amount: 599, status: "paid", dueDate: "2026-02-15", paidDate: "2026-02-14" },
    { invoiceNumber: `INV-2026-${nanoid(6).toUpperCase()}`, clientId: clients[1].id, subscriptionId: subs[1].id, amount: 299, status: "paid", dueDate: "2025-12-30", paidDate: "2025-12-28" },
    { invoiceNumber: `INV-2026-${nanoid(6).toUpperCase()}`, clientId: clients[2].id, subscriptionId: subs[2].id, amount: 299, status: "pending", dueDate: "2026-05-20" },
    { invoiceNumber: `INV-2026-${nanoid(6).toUpperCase()}`, clientId: clients[3].id, subscriptionId: subs[3].id, amount: 99, status: "overdue", dueDate: "2026-04-25" },
    { invoiceNumber: `INV-2026-${nanoid(6).toUpperCase()}`, clientId: clients[4].id, subscriptionId: subs[4].id, amount: 599, status: "paid", dueDate: "2026-03-10", paidDate: "2026-03-09" },
    { invoiceNumber: `INV-2026-${nanoid(6).toUpperCase()}`, clientId: clients[5].id, subscriptionId: subs[5].id, amount: 99, status: "paid", dueDate: "2025-05-01", paidDate: "2025-04-30" },
    { invoiceNumber: `INV-2026-${nanoid(6).toUpperCase()}`, clientId: clients[0].id, subscriptionId: subs[0].id, amount: 599, status: "pending", dueDate: "2026-08-15" },
  ]);
  ...
  await db.insert(notificationsTable).values([
    ...
    { userId: admin.id, title: "Invoice Overdue", message: "Seoul Street Kitchen invoice INV-2026 is 12 days overdue.", type: "billing", read: false },
